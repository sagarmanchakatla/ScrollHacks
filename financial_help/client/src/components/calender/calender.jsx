...
const Calendar = () => {
  ...
  const generateCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      dayElement.className =
        "text-center py-4 border cursor-pointer relative transition-colors duration-300 hover:bg-gray-200";
      dayElement.innerText = day;
      ...
      if (meetings[date]) {
        meetings[date].forEach((meeting, index) => {
          ...
          meetingElement.className =
            "absolute bg-red-300 rounded p-1 mt-1 text-xs";
  ...
  const scheduleMeeting = () => {
    ...
    setSelectedTime("");
    alert(
      `Meeting scheduled on ${selectedDate} at ${selectedTime}. Meeting Room ID: ${meetingRoomId}`
    );
  };
  ...
  return (
    <div className="bg-gray-100 flex flex-col items-center justify-center h-screen font-poppins ">
      <div className="lg:w-7/12 md:w-9/12 sm:w-10/12 mx-auto p-4">
        <h1 className="text-2xl font-bold text-center mb-4 mt-10">
          Meeting Scheduler
        </h1>
